...
function fillTable(x) {
  ...
  fetch("/api/" + x, {
    ...
    .then((res) => {
      ...
      let i;
      if (end % 40 == 0 && res.data.length >= end) i = end;
      else i = res.data.length;
      ...
      for (; i >= start; i--) {
        ...
        if (res.data[i].banned_by_name != "OverLegend Network") {
          authorImg.src = `https://mc-heads.net/avatar/${res.data[i].banned_by_name}/25`;
        } else {
        ...
        for (let k = 0; k < res.history.length && !stop; k++) {
          if (res.history[k].uuid == res.data[i].uuid) {
            ...
            playerImg.src = `https://mc-heads.net/avatar/${res.history[k].name}/25`;
        ...
        if (Number(startMinute) < 10) startMinute = "0" + startMinute;
        ...
        if (moderatorName == "OverLegend Network") moderatorName = "Console";
        ...
        if (res.data[i].removed_by_name == null && res.data[i].until == -1)
          ...
          if (Number(endHour) < 10) endHour = "0" + endHour;
          if (Number(endMinute) < 10) endMinute = "0" + endMinute;
          ...
          let rawPercent = Math.abs(
            ((today - res.data[i].time) / (res.data[i].until - res.data[i].time)) * 100
          );
          ...
          (res.data[i].removed_by_name == null || res.data[i].removed_by_name == "#expired") &&
          res.data[i].until < dtnow.getTime()
        ) {
          let dt2 = new Date(Number(res.data[i].until));
          let endDay = dt2.getDate();
          let endMonth = dt2.getMonth();
